...
class html {
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			...
			//评论跨站调用所需的JS文件
			if(substr($base_file, -10)=='old_index.html' && $count_number==3) {
				$copyjs = 1;
				$this->queue->add_queue('add',$base_file,$this->siteid);
			}
			//URLRULES
	...
	public function index() {
		if($this->siteid==1) {
			$file = PHPCMS_PATH.'old_index.html';
			//添加到发布点队列
			$this->queue->add_queue('edit','/old_index.html',$this->siteid);
		} else {
			$site_dir = $this->sitelist[$this->siteid]['dirname'];
			$file = $this->html_root.'/'.$site_dir.'/old_index.html';
			//添加到发布点队列
