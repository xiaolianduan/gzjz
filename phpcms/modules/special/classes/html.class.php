...
class html {
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if ($siteid>1) {
			...
			$file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/old_index.html';
		} else {
			$file = pc_base::load_config('system', 'html_root').'/special/'.$filename.'/old_index.html';
		}
		if(!$ispage) {
	...
	public function create_list($page = 1) {
		$siteid = get_siteid();
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(pc_base::load_config('system', 'html_root'), 1).'/special/old_index.html~'.$site_info['domain'].substr(pc_base::load_config('system', 'html_root'), 1).'/special/index-{$page}.html');
		//分站时计算路径
		if ($siteid>1) {
			if ($page==1) $file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/old_index.html';
			else $file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index-'.$page.'.html';
		} else {
			if ($page==1)  $file = pc_base::load_config('system', 'html_root').'/special/old_index.html';
